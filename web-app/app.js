...
const STORAGE = {
  ...
  webReminders: "campusflow-web-reminders",
...
createApp({
  ...
  data() {
    ...
    return {
      ...
      updateInfo: null,
      updateStatusText: isNativeRuntime() ? "从 GitHub Release 检查新版本" : "检查网站缓存与新版本",
  ...
  mounted() {
    ...
    if (this.webApp && this.reminderEnabled) this.restoreWebReminders();
  ...
  methods: {
    ...
    async clearCourseReminders(silent = true) {
      if (this.webApp) {
        this.webReminderTimers.forEach(timer => clearTimeout(timer));
        this.webReminderTimers = [];
        localStorage.removeItem(STORAGE.webReminders);
        if (!silent) this.notify("已清除网页提醒");
        return;
      }
      try {
    ...
    async scheduleCourseReminders() {
      ...
      try {
        ...
        if (this.webApp) {
          if (!("Notification" in window)) throw new Error("当前 Safari 不支持网页通知");
        ...
        if (this.webApp) {
          const serializable = scheduled.map(item => ({ ...item, schedule: { at: item.schedule.at.toISOString() } }));
          localStorage.setItem(STORAGE.webReminders, JSON.stringify(serializable));
          const maxDelay = 2147483647;
          scheduled.forEach(item => {
            const delay = item.schedule.at.getTime() - Date.now();
            if (delay <= 0 || delay > maxDelay) return;
            this.webReminderTimers.push(setTimeout(() => {
              new Notification(item.title, { body: item.body, icon: "./icons/icon-192.png" });
            }, delay));
          });
          this.notify(`已保存 ${scheduled.length} 条提醒；保持课序打开可准时通知`);
        } else {
    ...
    async checkForUpdate({ silent = false } = {}) {
      ...
      try {
        if (this.webApp) {
          ...
          this.updateStatusText = "网站缓存已更新";
          if (!silent) this.notify("已检查并更新网站缓存");
          return;
        }
    ...
    async webApiRequest(path, payload) {
      ...
      if (!response.ok) {
        const error = new Error(data?.detail || `服务返回异常（${response.status}）`);
    ...
    async checkSchoolStatus() {
      try {
        if (this.webApp) await this.webApiRequest("/api/whcibe/status");
        else await this.httpRequest("GET", LOGIN_URL, { responseType: "text" });
    ...
    restoreWebReminders() {
      if (!this.webApp || !("Notification" in window) || Notification.permission !== "granted") return;
      const reminders = readJson(STORAGE.webReminders, []);
      const maxDelay = 2147483647;
      this.webReminderTimers.forEach(timer => clearTimeout(timer));
      this.webReminderTimers = [];
      reminders.forEach(item => {
        const delay = new Date(item.schedule?.at).getTime() - Date.now();
        if (delay <= 0 || delay > maxDelay) return;
        this.webReminderTimers.push(setTimeout(() => {
          new Notification(item.title, { body: item.body, icon: "./icons/icon-192.png" });
        }, delay));
    ...
    showInstallGuide() {
      window.alert("在 iPhone Safari 中点击底部“分享”按钮，然后选择“添加到主屏幕”。首次安装后请从桌面上的“课序”图标打开。");
    },
    ...
    async syncFromSchool() {
      ...
      try {
        ...
        let newCourses;
        if (this.webApp) {
          const result = await this.webApiRequest("/api/whcibe/schedule", {
            username: this.syncForm.username,
            password: this.syncForm.password,
            semester: this.syncForm.semester
          });
          newCourses = Array.isArray(result?.data) ? result.data : [];
        } else {
          ...
          newCourses = rawCourses.map((course, index) => this.normalizeSchoolCourse(course, index));
        }
    ...
    async syncGradesFromSchool() {
      ...
      try {
        ...
        let rawGrades;
        if (this.webApp) {
          const result = await this.webApiRequest("/api/whcibe/grades", {
            username: this.gradeForm.username,
            password: this.gradeForm.password,
            semester: this.gradeForm.semester
          });
          rawGrades = Array.isArray(result?.data) ? result.data : [];
        } else {
          const [startYear, _endYear, term] = this.gradeForm.semester.split("-");
          rawGrades = await this.fetchSchoolGrades(startYear, term === "1" ? "3" : "12");
        }
    ...
    extractGradeComponents(item) {
      if (Array.isArray(item.components) && item.components.length) {
        return item.components.map(component => ({
          label: component.label || "成绩",
          value: String(component.value ?? component.score ?? ""),
    ...
    async openGrade(grade) {
      ...
      if (this.webApp) {
        this.notify("该课程暂未由教务系统开放分项成绩", "info");
        return;
      }
